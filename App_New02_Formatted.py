...
def set_background(image_file):
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <h4>Leaf It to AI: Diagnose Plant Diseases in Seconds!</h4>
        </div>
        """,
        unsafe_allow_html=True
    )
set_background("C:/Users/sriya/Jupyter/AI_Model/NewTrial/SoilBg2.jpg")
...
with st.sidebar.expander("Contact"):
    ...
    For queries, Contact:  
    <b>Sriya Kirti S</b> — <a href="mailto:sriyakswara@gmail.com">sriyakswara@gmail.com</a><br>
...
# Constants
IMAGE_SIZE = (128, 128)
MODEL_PATH = "C:/Users/sriya/Jupyter/AI_Model/NewTrial/model_ver12.keras"
...
def get_disease_info(label):
    ...
    return About_disease.get(label_cleaned)

# ---- Streamlit UI ----
st.title("🌿 Plant Disease Classification with Live Sensor Data")
st.markdown("<h6 style='color:#2b1d0e;'>View real-time environmental details with just one photo of your crop leaf!</h6>", unsafe_allow_html=True)
#st.markdown(
#    "<p style='color:#2b1d0e;'>Upload Image</p>",
#    unsafe_allow_html=True
#)
...
if uploaded_image is not None:
    ...
    if healthy_ranges:
        st.subheader("🌱 Recommended Healthy Conditions")
        col4, col5, col6 = st.columns(3)
        col4.metric("🌡️ Temperature Range", healthy_ranges["Temperature (°C)"])
        col5.metric("💧 Humidity Range", healthy_ranges["Humidity (%)"])
        col6.metric("🌱 Soil Moisture Range", healthy_ranges["Soil Moisture (%)"])
    else:
        st.warning("⚠️ Healthy ranges for this disease not found.")
    ...
    if info:
        ...
        st.markdown(f"**About the Disease:** {info['About the disease']}")
        st.markdown(f"**Symptoms:** {info['Symptoms']}")
        st.markdown(f"**Cure / Management:** {info['Cure']}")
    else:
        st.warning("⚠️ No detailed information available.")
